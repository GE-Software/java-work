...
public class Mergesort {
    ...
    public jobs[] mergesort(jobs[] a,int startindex, int endindex)
    {
        jobs[] left;
        jobs[] right;
        int middle;
        ...
        else
        {
           if(((endindex-startindex+1) % 2) ==0) // to take into account the array with odd number of elements.
           {
            ...
            left=new jobs[middle];
            right=new jobs[middle];
            //leftstartindex=startindex;
            //leftendindex=startindex+middle-1;
            //rightstartindex=startindex+middle;
     ...
     * @return 
     **/
    private jobs[] merge(jobs[]a,jobs[]b)
    {
        int leftlength=a.length;
        int rightlength=b.length;
        jobs[] result=new jobs[leftlength+rightlength];
        ...
        while((leftindex<leftlength) || (rightindex<rightlength))
        {
            ...
            {
                if(a[leftindex].score>=b[rightindex].score) // modified to calculate the descending order
                {
